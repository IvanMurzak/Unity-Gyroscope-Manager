...
This document describes how `IvanMurzak/Unity-Gyroscope-Manager` signs its
`extensions.unity.gyroscope.manager` package so the warning no longer appears in
Unity 6.3+.
...
`extensions.unity.gyroscope.manager-`, and uploads the signed `.tgz` as a `signed-upm-package`
workflow artifact.
...
```bash
gh secret set UPM_SERVICE_ACCOUNT_KEY_ID     --repo IvanMurzak/Unity-Gyroscope-Manager
gh secret set UPM_SERVICE_ACCOUNT_KEY_SECRET --repo IvanMurzak/Unity-Gyroscope-Manager
gh secret set UPM_ORG_ID                     --repo IvanMurzak/Unity-Gyroscope-Manager
```
...
If you publish via OpenUPM, OpenUPM's package listing for `extensions.unity.gyroscope.manager` likely
...
repository at `data/packages/extensions.unity.gyroscope.manager.yml`. Open a PR there changing:
...
1. Go to the release page for the new version and confirm a
   `extensions.unity.gyroscope.manager-<version>.tgz` asset is attached alongside the
...
2. Inspect the tarball locally to confirm it contains the signing attestation:
   ...
   curl -fsSL -o package.tgz \
     https://github.com/IvanMurzak/Unity-Gyroscope-Manager/releases/download/<version>/extensions.unity.gyroscope.manager-<version>.tgz
